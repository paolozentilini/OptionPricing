...
input_data->strike = 0.5;
//Solo per opzione esotica:
input_data->number_of_fixing_dates = 50;
...
int N = input_data->N;
int number_of_intervals = input_data->number_of_intervals;
...
double mean_value=0;
long double error=0;
//Su cpu creo il vettore traiettoria e le variabili clock_t
double* trajectory = new double[N*number_of_intervals];
...
int num_blocks = input_data->num_blocks;
int num_threads_per_block = input_data->num_threads_per_block;
int Nscenarios_for_thread = input_data->Nscenarios_for_thread;
double asset_price0 = input_data->asset_price0;
double risk_free_rate = input_data->risk_free_rate;
double sigma = input_data->sigma;
double strike = input_data->strike;
double delta_t = input_data->delta_t;
double lower_barrier_lim = input_data->lower_barrier_limit;
double upper_barrier_lim = input_data->upper_barrier_limit;
double notional_value = input_data->notional_value;
char option_type = input_data-> option_type;


for(int k=0; k<num_blocks; k++){
	for(int l=0; l < num_threads_per_block; l++){
		int tid = k*num_threads_per_block + l;
		int j= 4*tid;
		//Costruzione dell'array a 4 semi che deve essere usato per il funzionamento del generatore di numeri random
		unsigned int seeds_cpu[4];
		for(int i=0; i<4; i++){
			seeds_cpu[i] = seeds[j];
			j+=1;
		}
		//Cuore dell'algoritmo: utilizzo delle classi per il "pricing" dell'opzione finanziaria:
		CombinedGenerator generator(seeds_cpu);
		WienerProcess process(asset_price0, risk_free_rate, sigma);
		StandardPath path(number_of_intervals, delta_t, tid, &generator, &process, trajectory);
		PPC_Option option(strike, sigma, delta_t, notional_value, lower_barrier_lim, upper_barrier_lim, number_of_fixing_dates, fixing_dates);
		SimulationMonteCarlo simulation(Nscenarios_for_thread, tid, &path, &option);

		simulation.OptionPricing(mean, mean_squared);
	}
}
end_t = clock();
total_t = (double)(end_t - start_t) / CLOCKS_PER_SEC;

cout << setprecision(10) <<"Time taken by the cpu: " << total_t << " seconds" <<  endl;
